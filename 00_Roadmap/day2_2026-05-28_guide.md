...
> 文件名：`day2_2026-05-28_guide.md`  
> 日期：2026-05-28  
> 阶段：Phase 1 / W1 / Day 2  
> 今日主题：**完善场景文档 + DB Demo Studio 架构 Prompt 实战 + 第 2 条使用记录**  
> **实战代码库：** `02_DB_Demo_Studio/` · 模板：`01_AI_Dev_Workflow_Kit/prompts/`  
> 预计总时长：**2.5-3 小时**  
> API 策略：仅 DeepSeek → 见 [`learning_constraints.md`](learning_constraints.md)  
> 昨日复盘：[`day1_2026-05-27_workflow.md`](day1_2026-05-27_workflow.md)
...
**今日主任务：** 用 `prompts/architecture.md` 细化 **DB Demo Studio** 的 PoC #1 或 `demo-schema` 模块（不要脱离已确认的 [`02_DB_Demo_Studio/docs/architecture.md`](../02_DB_Demo_Studio/docs/architecture.md)）。

### 推荐任务（三选一）

| 选项 | 任务 | 适合原因 |
|---|---|---|
| **A（推荐）** | 为 `packages/demo-schema` + 样例 JSON 写架构补充（字段、校验、版本迁移） | 直接推进 PoC #1 |
| B | 为 `apps/web` 最小 Player 写模块划分与接口 | Day 3 可编码 |
| C | 智慧课堂助手等并行真实项目 | 面试故事；log 仍建议放 Studio 或 Kit |
...
2. 复制模板到 Cursor Chat
3. 填入：`02_DB_Demo_Studio/` 现有 docs + `@02_DB_Demo_Studio/docs/architecture.md` 中 DemoPackage 片段 + 你的细化问题（如「PoC Player 需要哪些 props？」）

**Step 2 — 生成并保存（25 min）**

保存 AI 输出到：
```
02_DB_Demo_Studio/logs/2026-05-28_demo_schema_architecture.md
```
（或 `01_AI_Dev_Workflow_Kit/logs/` 若选侧车任务）
...
## 时间块 4（30 min）— Code Review 闭环（可选脚本）

**任务：** 把 Day 2 在 `02_DB_Demo_Studio/`（或 Kit）的改动走一遍审查闭环。可用 `prompts/code_review.md` 手動审查，或 Kit 脚本。
...
- [ ] `02_DB_Demo_Studio/logs/2026-05-28_*` 架构设计产出
- [ ] `prompts/requirements.md` 和 `prompts/architecture.md` 有使用记录
- [ ] `workflow.md` 更新到 v0.2
- [ ] 第 2 条使用记录（Studio logs 优先；可选 ai_commit_review）
...
> 「按 day2_2026-05-28_guide.md 选项 A，帮我用 architecture.md 设计 Debug 辅助脚本，输出保存到 logs/」
