...
>
> **核心设计理念：** 所有功能都是"AI 对话交互"的延伸——用户不是在操作工具，而是在与 AI 对话协作完成教学演示的创造、优化和交付。多对话管理让用户可以并行推进多个备课任务，对话历史让每一次协作都被记录和复用。
...
**AI 协作式数据库教学演示工作台**——用户在与 AI 的多轮对话中，协作完成教学演示的生成、可视化、模拟器搭建、讲解词打磨、测验出题和效果验证。所有功能都可对话触发、对话优化、对话反馈。
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  教师打开 →  创建/选择对话 →  输入 SQL/知识点                      │
│       ↓                              ↓                           │
│   AI 回复（演示初稿 + 思维链）      ←  Agent 编排工具链            │
│       ↓                                                          │
│   教师在对话中:                                                    │
│     "把第三步的讲解改得更通俗"  →  AI 实时重写                      │
│     "加一个 B+树可视化"        →  AI 生成动画配置                  │
│     "给这一步出两道选择题"      →  AI 生成嵌入式测验                │
│     "对比 MySQL 和 PG 的代价"   →  AI 执行双引擎 EXPLAIN          │
│       ↓                                                          │
│  演示成品 = 对话的"物化产物"                                        │
│  切换对话 → 开始另一个备课任务                                       │
│  历史对话 → 可回溯、可复用、可对比                                   │
...
| 优先级 | 核心特征 | 对话交互方式 |
|---|---|---|
| **P0 即时演示** | EXPLAIN 6 阶段分步 | 对话输入 SQL → AI 流式生成 → 对话精修每步 |
| **P1 轻量可视化** | Mermaid/ASCII/ECharts | 对话"加个 ER 图"→ AI 生成 → "颜色改蓝"→ AI 调整 |
| **P2 专业模拟器** | SQL 分步/B+树/事务隔离 | 对话"模拟这个 JOIN 的执行"→ AI 构建交互式模拟器 |
...
│  │  ────────────────────────────────────────────────────     │   │
│  │  AI: 好的，我来分析这条 SQL...                              │   │
│  │      [思维链] 正在调用: sql_analyze → explain_mysql → ...  │   │
...
| 对比引擎 | "对比 MySQL 和 PG 的代价" | 调双引擎 EXPLAIN，生成对比讲解 |
...
| **F0.4 AI 协作交互** | 所有操作可对话触发，AI 响应含思维链+操作反馈+建议下一步 |
...
| 子项 | 对话交互方式 |
|---|---|
| **F1.1 SQL/知识点输入** | 用户在对话中输入 SQL 或描述知识点，AI 自动识别意图 |
| **F1.2 多模态输入** | 对话中可发 SQL/文字/图片 ER 图，AI 理解并处理 |
| **F1.3 流式生成+反馈** | AI 边生成边推送步骤预览，用户可随时打断"停，这里改一下" |
| **F1.4 演示类型协商** | AI 推荐最佳演示类型并说明理由，用户可对话调整 |
| **F1.5 迭代精修** | 用户对任一步骤对话反馈，AI 理解上下文后局部重写 |
| **F1.6 思维链可追溯** | AI 展示"我是怎么生成这个演示的"，用户可审查每一步推理 |
...
| 子项 | 对话交互方式 |
|---|---|
| **F3.1 SQL 模拟器** | "模拟这个 SQL 一步步怎么执行"→ AI 构建分步执行模拟器 |
...
### 3.6 阶段 EXPLAIN 展示（对话增强版）

每个 EXPLAIN 阶段不仅是静态展示，更是对话交互入口：
...
| 客户端 -> 服务端 | 说明 |
|---|---|
| `chat:message` | 发送消息（text/sql/image） |
...
| **第 3 周** | P2 SQL 模拟器 + Redis Pub/Sub 课堂广播 | 对话构建模拟器，教师端->学生端实时同步 |
